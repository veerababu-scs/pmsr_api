module RailsJwtAuth
  class ConfirmationsController < ApplicationController
    ...
    def show
      ...
      render_profile("Token Verified Successfully")
    end
    ...
    def create
      unless @user
        if RailsJwtAuth.avoid_email_errors
          return render_profile("Already Confirmed")
        else
          return render_422(RailsJwtAuth.email_field_name => [{error: :not_found}])
        end
      end

      @user.send_confirmation_instructions ? render_profile("Confirmation Email Has been to the User Email"): render_422(@user.errors.details)
    end

    # used to accept confirmation
    def update
      return render_profile "Invalid Token" unless @user

      @user.confirm ? render_profile("Email Confirmation Done"): render_422(@user.errors.details)
    end
