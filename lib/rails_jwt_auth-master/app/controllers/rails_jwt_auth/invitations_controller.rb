module RailsJwtAuth
  class InvitationsController < ApplicationController
    ...
    def show
      ...
      @user.expired_invitation_token? ? render_410 : render_profile("User Verified Token")
    end

    # used to invite a user, if user is invited send new invitation
    def create
      user = RailsJwtAuth.model.invite(invitation_create_params)
      user.errors.empty? ? render_profile("User Invited Successfully") : render_422(user.errors.details)
    end
    ...
    def update
      ...
      if @user.accept_invitation(invitation_update_params)
        render_profile("Invitation Accepted")
      else
